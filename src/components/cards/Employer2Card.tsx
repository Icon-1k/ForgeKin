...
function Stepper({ step }: { step: number }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: '24px 0 24px 0', background: '#fff', borderRadius: 6, width: '100%', padding: '8px 0' }}>
      {steps.map((s, idx) => (
        <React.Fragment key={s.label}>
          ...
          {idx < steps.length - 1 && <div style={{ flex: 1, borderTop: `2px solid ${idx < step ? accent : '#E5E5E5'}`, margin: '0 10px', minWidth: 25, marginTop: 14, marginBottom: -15 }} />}
        </React.Fragment>
      ))}
    </div>
  );
...
export default function Employer2Card() {
  return (
    <>
      ...
      <form style={{ marginTop: 12 }}>
        <div style={{ marginBottom: 17 }}>
          ...
          <div style={{ position: 'relative' }}>
            <svg viewBox="0 0 22 22" width="20" height="20" style={{ position: 'absolute', left: 12, top: 12, opacity: .6 }}>
        ...
        <button type="submit" style={{ background: accent, color: '#fff', border: 'none', borderRadius: 6, fontWeight: 600, fontSize: 18, letterSpacing: 0.03, padding: '12px 0', width: '100%', boxShadow: '0 1.5px 6px #ffbb2e25', marginBottom: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 11 }}>Create Account <span style={{ fontSize: 21, marginTop: -2, fontWeight: 700 }}>›</span></button>
      </form>
