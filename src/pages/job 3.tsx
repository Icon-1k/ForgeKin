...
/**
 * ForgeKinJobBoardDropdown.js
 * Single-file React component that recreates the job board with the open dropdown highlight (image 16).
 * - No external libraries required
 * - Inline styles for portability
 *
 * Usage:
 *  - Save as ForgeKinJobBoardDropdown.js
 *  - Import and render <ForgeKinJobBoardDropdown /> in your app
 */
...
function SearchBar({ query, setQuery, toggleFilters }: SearchBarProps) {
  return (
...
/* Custom select that opens and shows a highlighted item */
interface CustomOpenSelectProps {
  value: string | null;
  onChange: (value: string) => void;
  options: { value: string; label: string }[];
  placeholder?: string;
}
function CustomOpenSelect({ value, onChange, options, placeholder }: CustomOpenSelectProps) {
  const [open, setOpen] = useState(true); // start open to reflect screenshot
...
export default function ForgeKinJobBoardDropdown() {
