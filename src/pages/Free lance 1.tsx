...
import Freelance2Card from '../components/cards/Freelance1Card';
...
export default function ForgeKinStepper() {
  ...
  return (
    <div style={{ fontFamily: "Inter,sans-serif", background: "#F7F8F9", minHeight: "100vh" }}>
      ...
      {!showCard ? (
        <CenterCard maxWidth={650} padding="38px 45px 42px 45px" style={{ boxShadow: '0 3px 8px #0001' }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
            <button onClick={() => setShowCard(true)} style={{ flex: 1, background: accent, color: '#fff', border: 'none', padding: '12px 14px', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>Continue</button>
            <button onClick={() => navigate('/')} style={{ flex: 1, background: '#fff', color: '#232323', border: '1px solid #e6e6e6', padding: '12px 14px', borderRadius: 8, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
          </div>
        </CenterCard>
      ) : (
        <CenterCard
          variant={selected === 'freelancer' ? 'freelance' : 'employer'}
          maxWidth={650}
          padding="38px 45px 42px 45px"
          style={{ boxShadow: '0 3px 8px #0001' }}
          freelanceContent={selected === 'freelancer' ? <Freelance2Card /> : undefined}
          employerContent={selected === 'employer' ? <Employer1Card /> : undefined}
        />
      )}
      {/* Footer */}
